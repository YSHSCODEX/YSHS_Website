...
class CustomUser(AbstractUser):
    grade = models.IntegerField(verbose_name='학년',
                                choices=[(i, f'{i}학년') for i in range(1, 4)],)
    classNum = models.IntegerField(verbose_name='반',
                                   choices=[(i, f'{i}반') for i in range(1, 10)],)
    studentNum = models.IntegerField(verbose_name='출석번호',
                                     choices=[(i, f'{i}번') for i in range(1, 40)],)
    ...
    fAgreeCollectPersonInfor = models.BooleanField(default=False)
    # username 아이디
    # password1 비밀번호1
    # password2 비밀번호 확인
